...
namespace Ibis.Features._Plugins
{
    public class IbisEngine
    {
        ...
        internal async Task SpeakAsync(Message parentMessage, Messages.MessageTranslation message)
        {
            var config = SpeechConfig.FromSubscription(SpeechApiKey, "eastus");
            config.SpeechSynthesisLanguage = message.Language;
        ...
        {
            var otherLanguages = languages.Where(x => x.Id != message.Language).Select(x => x.Id).ToArray();
